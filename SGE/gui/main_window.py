...
from SGE.gui.users import GerenciarUsuarios
from SGE.gui.auditoria import HistoricoAuditoria
from SGE.gui.theme_manager import ThemeManager
...
class MainWindow(tk.Tk):
    def __init__(self, db, usuario_logado):
        ...
        self.theme_manager = ThemeManager(self, self.db)
        self.theme_manager.apply_theme('clam')
        ...
        self.after(60000, self._checar_alertas_periodicamente)
    ...
    def _criar_widgets(self):
        ...
        self.busca_produto_entry.bind('<KeyRelease>', self._filtrar_produtos)
        ...
        self.tree_produtos = ttk.Treeview(frame_produtos,
                                          columns=('ID', 'Código', 'Nome', 'Categoria', 'Quantidade', 'Preço Custo',
                                                   'Preço Venda', 'Quantidade Mínima'),
                                          show='headings')

        for col in ['ID', 'Código', 'Nome', 'Categoria', 'Quantidade', 'Preço Custo', 'Preço Venda',
                    'Quantidade Mínima']:
    ...
    def _verificar_alertas(self):
        produtos = self.db.executar('''SELECT nome, quantidade, quantidade_minima
                                       FROM produtos
                                       WHERE quantidade <= quantidade_minima''').fetchall()
        if produtos:
    ...
    def editar_produto(self):
        ...
        produto_data = self.db.executar(
            "SELECT id, codigo, nome, categoria_id, preco_custo, preco_venda, quantidade, quantidade_minima FROM produtos WHERE id = ?",
            (produto_id,)).fetchone()
        if produto_data:
    ...
    def exportar_movimentacoes_csv(self):
        ...
        try:
            movimentacoes = self.db.executar('''SELECT m.id,
                                                       p.nome,
                                                       IFNULL(c.nome, 'Sem Categoria'),
                                                       m.tipo,
                                                       m.quantidade,
            ...
            with open(file_path, 'w', newline='', encoding='utf-8') as file:
                writer = csv.writer(file)
                writer.writerow(
                    ['ID Movimentacao', 'Produto', 'Categoria', 'Tipo', 'Quantidade', 'Data', 'Observacao',
                     'Fornecedor', 'Usuario'])

                # Substitui valores None por 'N/A' para consistência
                processed_rows = []
                for row in movimentacoes:
                    processed_row = list(row)
                    if processed_row[7] is None:
                        processed_row[7] = 'N/A'
                    if processed_row[8] is None:
                        processed_row[8] = 'N/A'
